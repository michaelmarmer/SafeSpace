...
<html>
 ...
 <body>
   <nav class="navbar navbar-default" id="menubar">
     <form action="journal_log_submit.php" method="post" class="form-horizontal navbar-form navbar-left" role="form">
       <button name="return" id="return" class="btn btn-primary navbar-brand"><i class="fa fa-arrow-circle-left"></i></button>
       <a class="navbar-brand" id="journallog" >Journal Log</a>
   ...
   <tbody >
     <tr>
       <td><?php
         ...
         $sql = "SELECT journal_id FROM journal_entry WHERE username='".$_SESSION['username']."'";
         $result = $conn->query($sql);
         if ($result->num_rows > 0) {
           $index = 1;
             while($row = $result->fetch_assoc()) {
               $journal_id = $row["journal_id"];
               echo "<a name='journal_id' id='journal_id' onclick='newPage(\"$journal_id\")'>Journal Entry ".$index."</a><br>";
        ...
        <td><?php
          include 'database_connection.php';
          $sql = "SELECT dates FROM journal_entry WHERE username='".$_SESSION['username']."'";
          $result = $conn->query($sql);
          if ($result->num_rows > 0) {
              while($row = $result->fetch_assoc()) {
                $timestamp = $row["dates"];
